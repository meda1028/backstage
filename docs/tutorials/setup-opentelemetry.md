...
```json
"scripts": {
  "start": "backstage-cli package start --import ./instrumentation.js",
  ...
```
...
In your `Dockerfile` add the `--import` flag which points to the `instrumentation.js` file

```Dockerfile
# We need the instrumentation file inside the Docker image so we can use it with --import
...
// highlight-add-next-line
CMD ["node", "--import", "./instrumentation.js", "packages/backend", "--config", "app-config.yaml"]
```
